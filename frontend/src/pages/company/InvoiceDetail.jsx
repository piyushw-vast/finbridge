    ...
    <div className={`flex items-start py-3 border-b border-slate-50 last:border-0 ${flagged ? "bg-amber-50/60 -mx-2 px-2 rounded-lg" : ""}`}>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center">
          <span className={`text-sm font-medium flex-1 truncate ${value ? "text-slate-800 dark:text-slate-100" : "text-slate-300 italic"}`}>
            {value ?? "Not extracted"}
          </span>
    ...
    <div className="flex items-start py-3 border-b border-slate-50 last:border-0">
      ...
      <div className="flex-1 flex items-center gap-2">
        <select
          ...
          disabled={saving}
          className="flex-1 text-sm border border-slate-200 rounded-lg px-2.5 py-1.5 focus:outline-none focus:border-indigo-400 focus:ring-1 focus:ring-indigo-100 bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 disabled:opacity-60"
        >
  ...
  return (
    <Layout>
      <div className="px-8 py-8 max-w-5xl mx-auto">
        ...
        {/* Header card */}
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 shadow-sm p-6 mb-5 flex items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-bold text-slate-900 dark:text-white">{invoice.file_name}</h1>
              <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                ...
                {tx?.invoice_number && (
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 text-xs text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 px-2 py-0.5 rounded-full transition-all font-mono border border-slate-200 hover:border-indigo-200"
                    title="Copy invoice number"
                  >
                    {copied ? (
                      <svg className="w-3 h-3 text-emerald-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg>
                    ) : (
                    ...
                    {copied ? "Copied!" : tx.invoice_number}
                  </button>
                )}
        ...
        {aiSummary && !isProcessing && (
          <div className="mt-4 mb-2 flex items-start gap-2.5 bg-indigo-50 border border-indigo-100 rounded-xl px-4 py-3">
        ...
        {isProcessing ? (
          ...
          <div className="grid lg:grid-cols-3 gap-5 mt-5">
            {/* Left 2/3: Extracted Data + Field Confidence */}
            <div className="lg:col-span-2 space-y-5">
              {tx && (
                <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 shadow-sm p-6">
                  <div className="flex items-center justify-between mb-4">
                    <h2 className="text-base font-semibold text-slate-900 dark:text-white">Extracted Data</h2>
                  </div>
                  <div>
                      ...
                      <div className="mt-3 pt-3 border-t border-slate-50">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          <div className="bg-slate-50 rounded-lg px-3 py-2">
                            ...
                            <p className="text-sm font-semibold text-slate-700 dark:text-slate-200">₹{((tx.cgst ?? tx.tax_amount / 2)).toLocaleString("en-IN")}</p>
                          </div>
                          <div className="bg-slate-50 rounded-lg px-3 py-2">
                            <p className="text-xs text-slate-400">SGST (9%)</p>
                            <p className="text-sm font-semibold text-slate-700 dark:text-slate-200">₹{((tx.sgst ?? tx.tax_amount / 2)).toLocaleString("en-IN")}</p>
                          </div>
                        </div>
                      </div>
                    )}
                        ...
                        <table className="w-full text-xs">
                          ...
                          <tbody className="divide-y divide-slate-100">
                            {tx.line_items.map((item, i) => (
                              <tr key={i}>
                                <td className="px-4 py-2.5 text-slate-700 dark:text-slate-200">{item.description}</td>
                  ...
                  <div className="space-y-2">
                    {invoice.payment_method && (
                      <div className="flex justify-between text-xs">
                        ...
                        <span className="text-slate-800 dark:text-slate-100 font-semibold uppercase">{invoice.payment_method}</span>
                      </div>
                    )}
                    {invoice.utr_number && (
                      <div className="flex justify-between text-xs">
                        <span className="text-emerald-600 font-medium">UTR / Ref No.</span>
                        <span className="text-slate-800 dark:text-slate-100 font-mono font-semibold">{invoice.utr_number}</span>
                      </div>
                    )}
                    {invoice.bank_name && (
                      <div className="flex justify-between text-xs">
                        <span className="text-emerald-600 font-medium">Bank</span>
                        <span className="text-slate-800 dark:text-slate-100 font-semibold">{invoice.bank_name}</span>
                      </div>
                    )}
                  </div>
                </div>
              ...
              {invoice.reviewer_notes && (
                <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 shadow-sm p-6">
                  <h2 className="text-base font-semibold text-slate-900 dark:text-white mb-2">Accountant Notes</h2>
              ...
              {/* Field Confidence — inline below extracted data */}
              {tx && Object.keys(confidenceScores).length > 0 && (
                <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 shadow-sm p-5">
                  <div className="flex items-center justify-between mb-3">
                    <h2 className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Field Confidence</h2>
                  </div>
          ...
          {invoice.file_url && (
            <div className="mt-5 bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 shadow-sm p-6">
