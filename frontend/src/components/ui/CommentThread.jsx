  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
      <div className="px-6 py-4 border-b border-slate-100">
        <h2 className="text-base font-semibold text-slate-900 dark:text-white">Comments</h2>
          ...
          return (
            <div key={c.id} className={`flex gap-3 ${isMe ? "flex-row-reverse" : ""}`}>
              ...
              <div className={`max-w-[75%] ${isMe ? "items-end" : "items-start"} flex flex-col gap-1`}>
                ...
                <div className={`px-3.5 py-2.5 rounded-2xl text-sm leading-relaxed ${isMe ? "bg-indigo-600 text-white rounded-tr-sm" : "bg-slate-100 text-slate-800 dark:text-slate-100 rounded-tl-sm"}`}>
                  {c.message}
                </div>
              </div>
            </div>
